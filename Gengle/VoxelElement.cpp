...
VoxelElement::VoxelElement(ShaderConfig* shaders, VertexArray* vao, Buffer* array_buffer_in, Buffer* element_buffer_in) :
	GElement(shaders, vao, array_buffer_in, element_buffer_in)
{
	return;
}

void VoxelElement::Draw(void)
{
	glDrawElements(GL_TRIANGLES, this->elementCount, GL_UNSIGNED_INT, (void*)0);
	return;
}

void VoxelElement::Prepare(void)
{
	//// configure shaders
